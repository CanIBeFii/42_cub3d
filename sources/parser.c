...
/*   Updated: 2023/06/01 15:45:23 by fialexan         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	assign_path_to_info(t_map_info *info, char *line)
{
	...
	else if (ft_strncmp("F", line, 1) == 0 && info->ground_color == -1)
		info->ground_color = str_to_color(line + iter);
	else if (ft_strncmp("C", line, 1) == 0 && info->sky_color == -1)
...
int	has_all_info(t_map_info *info)
{
	if (info->north_texture == NULL && info->south_texture == NULL
		...
		return (0);
	if (info->sky_color == -1 && info->ground_color == -1)
		return (0);
	return (1);
}
